...
results_table = function(data, bmr_results){
  ...
  for(uni.task in uni.tasks){ #iterate through single tasks and fill results table
    ...
    results.table$p_rsq_corrected[results.table$task_id == uni.task & results.table$learner_id == uni.learner[3]] = p.adjust(c(results.table$p_rsq[results.table$task_id == uni.task & results.table$learner_id == uni.learner[3]]), n = 12, method = "holm")
    ...
    results.table$p_rsq_corrected[results.table$task_id == uni.task & results.table$learner_id == uni.learner[2]] = p.adjust(c(results.table$p_rsq[results.table$task_id == uni.task & results.table$learner_id == uni.learner[2]]), n = 12, method = "holm")
