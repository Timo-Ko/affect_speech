...
<!-- Model evaluation -->
Our prediction models were evaluated based on how accurate new (unseen) samples can be predicted. Throughout this manuscript, we report on the average Spearman correlation (rho) of true and predicted scores as measure of model fit. In the online supplements, we provide the coefficient of determination (*R*^2^), the mean absolute error (MAE), and the root mean squared error (RMSE) as additional performance measures for prediction models. To determine whether a model was predictive beyond chance (*alpha* = 0.05), we carried out variance-corrected (one-sided) t-tests comparing the *MAE* measures of prediction models from voice cues with those of the baseline models [@nadeauInferenceGeneralizationError2003]. We adjusted for multiple comparison  (in study 1: *n* = 4; in study 2: *n* = 6) via Holm correction. 
<!-- Software -->
